...
var globalClock;
var routineTimer;
// shared N-back response feedback (flashes briefly after SPACE press)
var nback_feedback;
var nback_feedback_onset;
async function experimentInit() {
  ...
  // Shared feedback component for all N-back routines
  // Displays "correct" (green) or "incorrect" (red) at bottom of screen for 300ms after SPACE press
  nback_feedback = new visual.TextBox({
    win: psychoJS.window,
    name: 'nback_feedback',
    text: '',
    placeholder: 'Type here...',
...
function practiceRoutineBegin(snapshot) {
  return async function () {
    ...
    practiceComponents.push(nback_feedback);
    // reset feedback state
    nback_feedback.setText('');
    nback_feedback.setAutoDraw(false);
    nback_feedback_onset = null;
  ...
  return async function () {
    ...
    if (response.status === PsychoJS.Status.STARTED) {
      ...
      if (_response_allKeys.length > 0) {
        ...
        // trigger feedback flash (only on SPACE press)
        if (nback_feedback_onset === null) {
          if (response.corr === 1) {
            nback_feedback.setText('correct');
            nback_feedback.setColor(new util.Color('green'));
          } else {
            nback_feedback.setText('incorrect');
            nback_feedback.setColor(new util.Color('red'));
          }
          nback_feedback.setAutoDraw(true);
          nback_feedback_onset = t;
        }
      }
    }
    
    // hide feedback after 300ms
    if (nback_feedback_onset !== null && t >= nback_feedback_onset + 0.3) {
      nback_feedback.setAutoDraw(false);
    }
...
function routine_2backRoutineBegin(snapshot) {
  return async function () {
    ...
    routine_2backComponents.push(nback_feedback);
    // reset feedback state
    nback_feedback.setText('');
    nback_feedback.setAutoDraw(false);
    nback_feedback_onset = null;
...
function routine_3backRoutineBegin(snapshot) {
  return async function () {
    ...
    routine_3backComponents.push(nback_feedback);
    // reset feedback state
    nback_feedback.setText('');
    nback_feedback.setAutoDraw(false);
    nback_feedback_onset = null;
...
function routine_4backRoutineBegin(snapshot) {
  return async function () {
    ...
    routine_4backComponents.push(nback_feedback);
    // reset feedback state
    nback_feedback.setText('');
    nback_feedback.setAutoDraw(false);
    nback_feedback_onset = null;
